#ifndef UPFNETWORKLIB_BUFFERS_HH
#define UPFNETWORKLIB_BUFFERS_HH
...
class BufferView {
  public:
    ...
    /// @brief Get a std::string out of the Length-Value (LV) composite string
    ///        in the BufferView.
    ///
    /// A LV string is a sequence of string components, filling all the
    /// BufferView. It's used in some S1AP NAS encondings.
    ///
    /// Each component specifies a component length (1st byte) followed
    /// by the component content (subsequent bytes), followed by other
    /// components until the end of the BufferView.
    ///
    /// @param separator The character that should be used to separate
    ///        string components in the result.
    ///
    std::string getLVCompositeString(char separator = '.') const {
        std::string result;
        result.reserve(mSize);

        // Note: bounds already checked before.
        const auto *p = mPtr;
        const auto *e = mPtr + mSize;
        auto r = std::back_inserter(result);
        char l;

        while (p < e) {
            // Take the component length
            l = *p;
